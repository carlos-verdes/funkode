...
object TodoApp extends ZIOAppDefault:
  ...
  def app: RIO[TodoService, Boolean] = for
    ...
    shouldExit <-
      if todoDescription.trim.isEmpty || todoDescription.trim.toLowerCase == EXIT then ZIO.succeed(false)
      else createTodo(todoDescription) *> app
  yield shouldExit

  def run =
    app.provide(TodoService.live, TodoRepository.inMemory)
