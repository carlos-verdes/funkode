...
trait GenericArangoClient[F[_], Encoder, Decoder] {
     ...
     */
    def execute[O: Decoder](header: ArangoRequest.Header): F[ArangoResponse[O]]
     ...
     * @return arango response
     */
    def execute[P: Encoder, O: Decoder](request: ArangoRequest[P]): F[ArangoResponse[O]]
...
object GenericArangoClient {
   ...
   * @tparam D decoder
   */
  abstract class Impl[F[_], Encoder, Decoder](
      configuration: ArangoConfiguration)(implicit F: MonadThrow[F], L: MessageLogger[F])
      extends ArangoClient[F] {

    def encode[R: Encoder](request: R): F[BitVector]
    def decode[R: Decoder](bits: BitVector): F[R]
    ...
    override def execute(header: ArangoRequest.Header): F[ArangoResponse[O]] =
      for {
        _  <- L.trace(show"$traceREQ header $header")
