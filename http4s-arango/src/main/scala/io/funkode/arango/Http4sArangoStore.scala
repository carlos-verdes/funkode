...
class Http4sArangoStore[F[_]](clientR: Resource[F, Client[F]])(implicit F: Sync[F]) extends JsonStoreDsl[F] {

  import ColKey._
  import rest.error._
  import rest.resource._

  def execute[R](command: (Client[F]) => F[R]): F[R] = handleArangoErrors(clientR.use(command))
