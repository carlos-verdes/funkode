...
package object arango {
  ...
  type JsonStoreDsl[F[_]] = HttpStoreDsl[F, Encoder, Decoder]
  ...
  def buildEdgeDoc(key: String, leftUri: Uri, rightUri: Uri): Json =
    Json.obj(
        "_key" -> Json.fromString(key),
        "_from" -> Json.fromString(leftUri.path.toString().substring(1)),
        "_to" -> Json.fromString(rightUri.path.toString().substring(1))
    )
